...
namespace VRTK.Highlighters
{
    ...
    public class VRTK_MaterialColorSwapHighlighter : VRTK_BaseHighlighter
    {
        ...
        private int alb = Shader.PropertyToID("_Color"); 
        ...
        protected Dictionary<string, Color> originalRendererEmissionColor = new Dictionary<string, Color>();
        protected Dictionary<string, Color> originalRendererAlbedoColor = new Dictionary<string, Color>();
        ...
        public override void Initialise(Color? color = null, Dictionary<string, object> options = null)
        {
            ...
            originalRendererEmissionColor = new Dictionary<string, Color>();
            originalRendererAlbedoColor = new Dictionary<string, Color>();
            faderRoutines = new Dictionary<string, Coroutine>();
            resetMainTexture = GetOption<bool>(options, "resetMainTexture");
            ResetHighlighter();
            if (linked) linked.Initialise(color ,options );
            initialized = true;
        }

        private bool initialized;
        [SerializeField] private VRTK_MaterialColorSwapHighlighter linked;
        /// <summary>
        ...
        public override void ResetHighlighter()
        {
            ...
            if (linked) linked.ResetHighlighter();
        }
        ...
        {
            if (linked) linked.Highlight(color,duration);
            if (color == null)
            {
                return;
            }
            ChangeToHighlightColor((Color)color, duration);
            //Debug.Log("HighLight call");
        }
        ...
        {
            if (linked) linked.Unhighlight(color, duration);
            ...
            foreach (Renderer renderer in GetComponentsInChildren<Renderer>(true)) {
                foreach (var mat in renderer.materials){
                    var na = mat.GetInstanceID().ToString();
                    if (originalRendererAlbedoColor.ContainsKey(na)) {
                        mat.color= originalRendererAlbedoColor[na];
                    }
                    if (originalRendererEmissionColor.ContainsKey(na)) {
                        mat.SetColor(emis, originalRendererEmissionColor[na]);
                    }
                }
            }
        ...
        protected virtual void StoreOriginalMaterials()
        {
            ...
            originalRendererEmissionColor.Clear();
            foreach (Renderer renderer in GetComponentsInChildren<Renderer>(true))
            {
                var objectReference = renderer.gameObject.GetInstanceID().ToString();
                ...
                foreach (var mat in renderer.materials) {
                    var na=mat.GetInstanceID().ToString();
                    if (mat.HasProperty(alb))
                        originalRendererAlbedoColor.Add(na, mat.color);
                    if (mat.HasProperty(emis)) {
                        originalRendererEmissionColor.Add(na,mat.GetColor(emis));
                    }
        ...
        protected virtual void ChangeToHighlightColor(Color color, float duration = 0f)
        {
            foreach (Renderer renderer in GetComponentsInChildren<Renderer>(true))
            {
                ...
                for (int i = 0; i < renderer.materials.Length; i++)
                {
                    ...
                    {
                        renderer.material.SetTexture("_MainTex", new Texture2D(128,128));
                    }
