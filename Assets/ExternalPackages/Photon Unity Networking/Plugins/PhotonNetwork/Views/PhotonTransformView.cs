...
public class PhotonTransformView : MonoBehaviour, IPunObservable
{
    ...
    void UpdatePosition()
    {
        ...
        if(localSync) transform.localPosition = this.m_PositionControl.UpdatePosition(transform.localPosition);
        else transform.position = this.m_PositionControl.UpdatePosition(transform.position);
    }
    ...
    void UpdateRotation()
    {
        ...
        if (localSync) transform.localRotation = this.m_RotationControl.GetRotation(transform.localRotation);
        else transform.rotation = this.m_RotationControl.GetRotation(transform.rotation);
    }
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        if (stream.isReading == true)
        {
			...
			if (m_firstTake)
			{
				...
				{
				    if (localSync) this.transform.localPosition = this.m_PositionControl.GetNetworkPosition();
                    else this.transform.position = this.m_PositionControl.GetNetworkPosition();
				}

				if (this.m_RotationModel.SynchronizeEnabled)
				{
                    if(localSync) this.transform.localRotation = this.m_RotationControl.GetNetworkRotation();
                    else this.transform.rotation = this.m_RotationControl.GetNetworkRotation();
				}
